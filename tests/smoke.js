...
try {
  ...
  await page.evaluate(() => window.dispatchEvent(new CustomEvent('reboot:drive-status', { detail: { state: 'syncing', dirty: false } })));
  if (!(await page.locator('#syncNotice').isHidden())) throw new Error('A technical refresh sync must not display a leave-warning without a new local entry');
  ...
  if ((await page.locator('input[name="funding"]:checked').count()) !== 0) throw new Error('Health funding must replace the other funding choice while preserving expense nature');
  ...
  await page.locator('#saveAnotherExpenseButton').click();
  if (!(await page.locator('#expenseDialog').isVisible()) || await page.locator('#expenseAmount').inputValue() !== '') throw new Error('Save-and-add-another must keep a clean expense form open');
  const batchSaved = await page.evaluate(async () => Boolean((await RebootSecureStorage.read('reboot-local-v1', 'reboot-local-v1'))?.expenses?.some(item => item.label === 'Test série' && !item.deletedAt)));
  if (!batchSaved) throw new Error('Save-and-add-another must persist the previous entry locally before resetting the form');
  await page.locator('#expenseDialog .close-button').click();
  acceptConfirmation(page);
  await page.locator('#expenseList .expense-item', { hasText: 'Test série' }).locator('[data-delete]').click();
  await page.locator('#addExpenseButton').click();
  ...
  await page.locator('#expenseList .expense-item', { hasText: 'Test correction' }).locator('[data-edit-expense]').click();
  ...
  await page.locator('#addRefundButton').click();
  await page.locator('#refundAmount').fill('4');
  await page.locator('#refundLabel').fill('Ajout historique');
  ...
  await assertContains(page.locator('#trackingView'), '6,00');
  await assertContains(page.locator('#trackingView'), '565,15');
  await page.goto(`${baseUrl}/app.html#week`, { waitUntil: 'networkidle' });
  ...
  acceptConfirmation(page);
  await page.locator('#historyList [data-delete-refund]').click();
  acceptConfirmation(page);
