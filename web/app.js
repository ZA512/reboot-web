...
const fullDate = value => new Intl.DateTimeFormat('fr-FR', { day: 'numeric', month: 'short', year: 'numeric' }).format(new Date(`${value}T12:00:00`));
...
function saveState(options = {}) {
  ...
  if (waitForDrive || options.userInitiated) syncNoticeArmed = Boolean(window.RebootDrive?.config?.().configured);
...
function renderSyncNotice(syncState, drive, pendingCount) {
  ...
  if (syncState === 'sync_pending') {
    if (!syncNoticeArmed) { notice.classList.add('hidden'); return; }
    syncWasPending = true; notice.classList.remove('hidden');
    text.textContent = `${pendingCount} modification${pendingCount > 1 ? 's' : ''} enregistrée${pendingCount > 1 ? 's' : ''} ici. Envoi vers Drive…`;
    return;
  }
  if (syncState === 'syncing' && syncWasPending) {
    notice.classList.remove('hidden'); text.textContent = 'Envoi vers Google Drive… gardez REBOOT ouvert un instant.';
    return;
  }
  if (['sync_error', 'sync_delayed', 'reauth_required'].includes(syncState)) {
    if (!syncNoticeArmed && !syncWasPending) { notice.classList.add('hidden'); return; }
  ...
  if (syncState === 'connected_idle' && !drive?.dirty && syncWasPending) {
    syncWasPending = false; syncNoticeArmed = false; notice.classList.add('success'); notice.classList.remove('hidden'); text.textContent = 'Synchronisé — vous pouvez quitter REBOOT.';
...
function renderWeek() {
  ...
  if (!cycle.configured) {
    $('#remaining').textContent = '—'; $('#budgetTotal').textContent = 'À définir'; $('#cycleDates').textContent = 'Non configuré'; $('#daysLeft').textContent = 'Préparez le budget'; $('#dailyGuide').textContent = '—'; $('#balanceTrack').style.width = '0%'; $('#weekSpent').classList.add('hidden'); $('#currentWeekChartButton').classList.add('hidden'); $('#reserveDeduction').classList.add('hidden');
  } else {
    ...
    $('#weekSpent').classList.remove('hidden'); $('#currentWeekChartButton').classList.remove('hidden'); $('#weekSpentTotal').textContent = formatMoney(cycle.expenseMinor); $('#weekReservedTotal').textContent = formatMoney(cycle.transferMinor); $('#weekRefundTotal').textContent = formatMoney(cycle.refundMinor); $('#weekNetTotal').textContent = formatMoney(cycle.spentMinor - cycle.refundMinor);
...
const WEEK_CHART_CATEGORIES = [
  { key: 'necessary', label: 'Nécessaire' },
  { key: 'pleasure', label: 'Plaisir' },
  { key: 'unexpected', label: 'Imprévu' },
  { key: 'postponable', label: 'Reportable' },
...
function openWeekChart(startDate) {
  if (!startDate) return;
  const chart = weekChartBreakdown(startDate), visible = WEEK_CHART_CATEGORIES.filter(category => chart.values[category.key] > 0), maximum = Math.max(1, ...visible.map(category => chart.values[category.key]));
  ...
  $('#weekChartAdjustments').classList.toggle('hidden', !adjustments.length); $('#weekChartAdjustments').textContent = adjustments.join(' · ');
  $('#weekChartDialog').showModal();
}
function bindWeekChartActions(root = document) { root.querySelectorAll('[data-week-chart]').forEach(button => button.onclick = () => openWeekChart(button.dataset.weekChart)); }
function renderTracking() {
  ...
  $('#trackingList').innerHTML = rows.length ? `<div class="tracking-head"><span>Semaine</span><span>Budget</span><span>Dépensé</span><span>Écart</span></div>${rows.map(row => `<article class="tracking-row"><span class="tracking-week-cell"><span>${shortDate(row.startDate)} → ${shortDate(row.endDate)}</span><button class="week-chart-trigger" type="button" data-week-chart="${row.startDate}" aria-label="Voir le graphique de la semaine du ${shortDate(row.startDate)}" title="Graphique de la semaine">▥</button></span><span>${formatMoney(row.budgetMinor)}</span><span>${formatMoney(row.spentMinor)}</span><strong class="${row.resultMinor < 0 ? 'negative' : 'positive'}">${row.resultMinor >= 0 ? 'Gain +' : 'Dépassement −'} ${formatMoney(Math.abs(row.resultMinor))}</strong></article>`).join('')}` : '';
  bindWeekChartActions($('#trackingList'));
...
function entryCycleStart(entry) {
  if (entry.displayCycleStart) return entry.displayCycleStart;
  return state.configured && state.rebootDay !== null && state.rebootDay !== undefined ? allocationCycleStart(entry.date) : entry.date;
}
function entryHtml(entry) {
  const type = movementType(entry), isRefund = entry.entryType === 'refund';
  const duplicateNote = entry.duplicateMatches?.length ? `<div class="possible-duplicate">Même montant saisi aussi ${entry.duplicateMatches.length === 1 ? 'le' : 'les'} ${entry.duplicateMatches.map(item => shortDate(item.date)).join(', ')}</div>` : '';
  if (isRefund) return `<article class="expense-item refund movement-${type.key}"><div class="expense-symbol">+</div><div><div class="expense-label">${escapeHtml(entry.label)}</div><div class="expense-meta"><span class="movement-type">${type.label}</span><span>${shortDate(entry.date)}</span></div>${duplicateNote}</div><div class="expense-amount positive">+ ${formatMoney(entry.amountMinor)}</div><div class="expense-actions"><button class="delete-expense" data-edit-refund="${entry.id}">Modifier</button><button class="delete-expense" data-delete-refund="${entry.id}">Supprimer</button></div></article>`;
  ...
  const allocationCount = Number(entry.allocationCount || activeAllocations(entry.id).length), spreadLabel = allocationCount > 1 ? ` · étalée sur ${allocationCount} semaines · total payé ${formatMoney(entry.amountMinor)}` : '';
  const allocationLabel = allocationCount > 1 && entry.allocationSequence ? ` · part ${entry.allocationSequence}/${allocationCount}` : '';
  const displayedAmount = entry.displayAmountMinor ?? entry.amountMinor;
  return `<article class="expense-item movement-${type.key}"><div class="expense-symbol">−</div><div><div class="expense-label">${escapeHtml(entry.label)}</div><div class="expense-meta"><span class="movement-type">${type.label}</span><span>${shortDate(entry.date)}${fundingDetail ? ` · ${fundingDetail}` : ''}${allocationLabel}${spreadLabel}${nature ? ` · ${nature}` : ''}</span></div>${duplicateNote}</div><div class="expense-amount">− ${formatMoney(displayedAmount)}</div><div class="expense-actions"><button class="delete-expense" data-edit-expense="${entry.id}">Modifier</button><button class="delete-expense" data-delete="${entry.id}">Supprimer</button></div></article>`;
}
...
function displayEntries() {
  const currentStart = state.configured && state.rebootDay !== null && state.rebootDay !== undefined ? allocationCycleStart(dateKey(new Date())) : '';
  const expenses = state.expenses.filter(item => !item.deletedAt).flatMap(expense => {
    const allocations = expense.funding === 'weekly' ? activeAllocations(expense.id).sort((a, b) => a.cycleStart.localeCompare(b.cycleStart)) : [];
    if (allocations.length <= 1) return [{ ...expense, entryType: 'expense', displayCycleStart: allocations[0]?.cycleStart || '' }];
    const visible = allocations.filter(allocation => !currentStart || allocation.cycleStart <= currentStart);
    if (!visible.length) return [{ ...expense, entryType: 'expense', displayCycleStart: allocationCycleStart(expense.date), allocationCount: allocations.length }];
    return visible.map(allocation => ({ ...expense, entryType: 'expense', displayCycleStart: allocation.cycleStart, displayAmountMinor: Number(allocation.amountMinor || 0), allocationSequence: Number(allocation.sequence || 0), allocationCount: allocations.length }));
  });
  const refunds = state.refunds.filter(item => !item.deletedAt).map(item => ({ ...item, entryType: 'refund' }));
  return [...expenses, ...refunds].sort((a, b) => `${entryCycleStart(b)}${b.date}${b.createdAt || ''}${b.allocationSequence || 0}`.localeCompare(`${entryCycleStart(a)}${a.date}${a.createdAt || ''}${a.allocationSequence || 0}`));
}
function possibleDuplicateMap() {
  const entries = allEntries(), matches = new Map(), key = entry => `${entry.entryType}:${entry.id}`;
  for (let index = 0; index < entries.length; index += 1) {
    for (let candidateIndex = index + 1; candidateIndex < entries.length; candidateIndex += 1) {
      const entry = entries[index], candidate = entries[candidateIndex];
      if (entry.entryType !== candidate.entryType || Number(entry.amountMinor || 0) !== Number(candidate.amountMinor || 0) || Math.abs(RebootBudgetEngine.daysBetween(entry.date, candidate.date)) > 5) continue;
      matches.set(key(entry), [...(matches.get(key(entry)) || []), candidate]);
      matches.set(key(candidate), [...(matches.get(key(candidate)) || []), entry]);
    }
  }
  return matches;
}
function groupedEntriesHtml(entries) {
  if (!state.configured || state.rebootDay === null || state.rebootDay === undefined) return entries.map(entryHtml).join('');
  const currentStart = allocationCycleStart(dateKey(new Date())); let previousStart = '';
  return entries.map(entry => {
    const start = entryCycleStart(entry); let heading = '';
    if (start !== previousStart) { const label = start === currentStart ? 'Semaine en cours' : start < currentStart ? 'Semaine terminée' : 'Semaine à venir'; heading = `<div class="movement-period"><span>${label}</span><strong>${weekPeriodLabel(start)}</strong></div>`; previousStart = start; }
    return heading + entryHtml(entry);
  }).join('');
}
...
function renderCurrentExpenses(cycle) {
  const currentStart = cycle.configured ? dateKey(cycle.start) : '', visibleEntries = displayEntries();
  const entries = !cycle.configured ? [] : visibleEntries.filter(entry => entryCycleStart(entry) === currentStart).slice(0, 5);
  ...
  const historical = cycle.configured ? visibleEntries.filter(entry => entryCycleStart(entry) !== currentStart) : []; $('#historySection').classList.toggle('hidden', !historical.length); $('#historyList').innerHTML = groupedEntriesHtml(historical); bindEntryActions($('#historyList'));
}
function renderMovements() {
  const cycle = cycleInfo(), query = normalizeMovementSearch($('#movementSearch').value), compactQuery = query.replace(/[\s€]/g, '').replace(',', '.'), duplicates = possibleDuplicateMap(), duplicateKey = entry => `${entry.entryType}:${entry.id}`;
  let entries = movementFilter === 'duplicates' ? allEntries().filter(entry => duplicates.has(duplicateKey(entry))).map(entry => ({ ...entry, duplicateMatches: duplicates.get(duplicateKey(entry)) })) : displayEntries();
  if (movementFilter === 'week' && cycle.configured) entries = entries.filter(entry => entryCycleStart(entry) === dateKey(cycle.start));
  ...
  if (query) entries = entries.filter(entry => {
    ...
    const sign = entry.entryType === 'refund' ? '+' : '-', amounts = [...new Set([entry.displayAmountMinor ?? entry.amountMinor, entry.amountMinor].map(value => (Math.abs(Number(value || 0)) / 100).toFixed(2)))];
    return text.includes(query) || amounts.flatMap(decimal => [decimal, `${sign}${decimal}`]).some(token => token.includes(compactQuery));
  });
  $('#allMovementsList').innerHTML = groupedEntriesHtml(entries); $('#movementsEmpty').classList.toggle('hidden', Boolean(entries.length));
  $('#movementsEmpty h3').textContent = movementFilter === 'duplicates' ? 'Aucun doublon apparent' : 'Aucun résultat'; $('#movementsEmpty p').textContent = movementFilter === 'duplicates' ? 'Aucun montant identique saisi à moins de cinq jours d’intervalle.' : 'Modifiez votre recherche ou ajoutez un montant.';
  bindEntryActions($('#allMovementsList'));
}
...
function ensureShortcutField() {
  ...
  if (!$('#saveAnotherExpenseButton')) {
    const button = document.createElement('button'); button.className = 'button button-secondary save-another-button'; button.id = 'saveAnotherExpenseButton'; button.type = 'submit'; button.value = 'save-another'; button.textContent = 'Enregistrer et ajouter';
    $('#saveExpenseButton').before(button);
  }
  const healthChoice = $('#expenseHealth').closest('label');
  healthChoice.className = 'funding-choice funding-choice-health'; healthChoice.title = 'Dépense remboursable financée par la réserve Santé';
  $('.funding-choices').append(healthChoice); $('.nature-help')?.classList.add('hidden');
  const healthLabel = $('#expenseHealth').nextElementSibling;
  healthLabel.innerHTML = '<strong>✚ Santé</strong><small>remboursable</small>';
  $('#expenseHealth').setAttribute('aria-label', 'Dépense Santé remboursable');
}
...
function openExpenseDialog(expense = null, health = false) {
  ...
  $('#expenseHealth').checked = isHealth; if (isHealth) $$('input[name="funding"]').forEach(input => { input.checked = false; }); $('#fundingField').classList.remove('hidden'); $('#expenseDialogKicker').textContent = expense ? 'Correction' : isHealth ? 'Réserve Santé' : 'Nouvelle dépense'; $('#expenseDialogTitle').textContent = expense ? 'Modifier le montant' : isHealth ? 'Ajouter une dépense Santé' : 'Ajouter un montant'; $('#saveExpenseButton').textContent = expense ? 'Enregistrer les modifications' : 'Enregistrer'; $('#saveAnotherExpenseButton').classList.toggle('hidden', Boolean(expense)); $('#expenseDate').value = expense?.date || dateKey(new Date());
...
function toggleReserveChoice() {
  ...
  $('#fundingField').classList.remove('hidden'); $('#reserveChoice').classList.toggle('hidden', health || funding !== 'reserve'); $('#trackedChargeChoice').classList.toggle('hidden', health || funding !== 'tracked');
  ...
  $('#fundingHelp').textContent = health ? 'Utilise la réserve Santé sans réduire directement la semaine.' : help[funding] || ''; updateSpreadControls();
}
...
function applyShortcut(shortcut) {
  ...
  if (shortcut.health) $$('input[name="funding"]').forEach(input => { input.checked = false; });
  else { const funding = document.querySelector(`input[name="funding"][value="${shortcut.fundingChoice || shortcut.funding || 'weekly'}"]`); if (funding) funding.checked = true; }
...
async function deleteShortcut(id) {
  ...
  const before = snapshot(shortcut), now = new Date().toISOString(); shortcut.deletedAt = now; shortcut.updatedAt = now; recordEvent('deleted', 'shortcut', shortcut.id, before, shortcut); await saveState({ userInitiated: true }); renderRecentLabels();
}
...
async function saveExpense(event) {
  ...
  const saveButton = event.submitter || $('#saveExpenseButton'), defaultButtonText = saveButton.textContent, buttons = [$('#saveExpenseButton'), $('#saveAnotherExpenseButton')]; buttons.forEach(button => { button.disabled = true; });
  saveButton.textContent = saveAnother ? 'Ajouté — suivant…' : window.RebootDrive?.config?.().configured ? 'Envoi vers Drive…' : 'Enregistrement…';
  try {
    if (saveAnother) { await saveState({ userInitiated: true }); render(); openExpenseDialog(null, health); }
    else { await saveState({ waitForDrive: true }); $('#expenseDialog').close(); render(); }
  } finally { buttons.forEach(button => { button.disabled = false; }); saveButton.textContent = defaultButtonText; }
}
function deleteExpense(id) { const expense = state.expenses.find(item => item.id === id); if (!expense) return; const allocations = activeAllocations(id), label = expense.label || 'cette dépense'; const message = allocations.length > 1 ? `Supprimer « ${label} » et ses ${allocations.length} affectations hebdomadaires ?` : `Supprimer « ${label} » ?`; if (!confirm(message)) return; const before = snapshot(expense), deletedAt = new Date().toISOString(); expense.deletedAt = deletedAt; expense.updatedAt = deletedAt; allocations.forEach(item => { item.deletedAt = deletedAt; item.updatedAt = deletedAt; }); recordEvent('deleted', 'expense', id, before, expense); saveState({ userInitiated: true }); render(); }
...
function closeReserve(id) { const reserve = state.reserves.find(item => item.id === id); if (!reserve || !confirm(`Terminer « ${reserve.name} » ?`)) return; const before = snapshot(reserve); reserve.closedAt = new Date().toISOString(); recordEvent('closed', 'reserve', id, before, reserve); saveState({ userInitiated: true }); render(); }
...
async function saveCharge(event) {
  ...
  syncNoticeArmed = Boolean(window.RebootDrive?.config?.().configured);
...
$('#addExpenseButton').onclick = () => openExpenseDialog(); $('#emptyAddButton').onclick = () => openExpenseDialog(); $('#movementAddButton').onclick = () => openExpenseDialog(); $('#addHealthExpenseButton').onclick = () => openExpenseDialog(null, true); $('#addRefundButton').onclick = () => openRefundDialog(); $('#addHealthRefundButton').onclick = () => openRefundDialog(null, true); $('#settingsButton').onclick = openSettings; $('#addReserveButton').onclick = () => openReserveDialog(); $('#addChargeButton').onclick = () => openChargeDialog(); $('#rebalanceHealthButton').onclick = openRebalanceDialog; $('#applyRecommendedBudget').onclick = applyRecommendedBudget; $('#currentWeekChartButton').onclick = () => { const cycle = cycleInfo(); if (cycle.configured) openWeekChart(dateKey(cycle.start)); };
...
$$('input[name="funding"]').forEach(input => input.onchange = () => { if (input.checked) $('#expenseHealth').checked = false; toggleReserveChoice(); }); $('#expenseHealth').onchange = () => { if ($('#expenseHealth').checked) $$('input[name="funding"]').forEach(input => { input.checked = false; }); else document.querySelector('input[name="funding"][value="weekly"]').checked = true; toggleReserveChoice(); }; $('#refundHealth').onchange = () => { const health = $('#refundHealth').checked; $('#refundApplyField').classList.toggle('hidden', health); if (health) $('#refundApply').checked = false; }; $('#refundExpense').onchange = () => { const expense = state.expenses.find(item => item.id === $('#refundExpense').value); if (expense?.health) { $('#refundHealth').checked = true; $('#refundHealth').dispatchEvent(new Event('change')); } };
...
(async function init() {
  try {
    if ('serviceWorker' in navigator) navigator.serviceWorker.register('./sw.js?v=66', { updateViaCache: 'none' }).catch(() => {});
